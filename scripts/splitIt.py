...
def split_last_message(entry, split_on, key):
    ...
    if list_size == 0:
        ...
        if key in new_entry:
            new_entry[key] += " " + response_content
        else:
            new_entry[key] = response_content
        return [new_entry]
...
def main(args):
    ...
    for entry in input_data:
        ...
        new_entries = split_last_message(entry, args.split_on, args.key)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-key', required=True, help='New key to assign the split parts to')
    args = parser.parse_args()
    #import shlex ; args = parser.parse_args(shlex.split("-input_json genre_responses_trimmed.json -split_on 'and' -key 'partytime'"))    
